...
def change_rating_flat(user_id, penalty):
    """Используется для штрафов при грубых нарушениях или отказах"""
    conn = sqlite3.connect("taskearn_v29.db")
    with conn:
        cursor = conn.cursor()
...
def approve_task(task_id, total_reward, worker_id, rating_val=None):
    """Выплата воркеру + динамический перерасчет рейтинга по формуле среднего арифметического"""
    now_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    ...
    with conn:
        cursor = conn.cursor()
        # 1. Меняем статус задачи и переводим деньги
        ...
        # 2. Если выставлена оценка, пересчитываем рейтинг воркера
...
# БЕЗОПАСНОСТЬ: Авторизация через query_params удалена для предотвращения несанкционированного доступа.
...
if user_data['role'] != 'admin':
    ...
    # ЭКРАН 3: КОНТРОЛЬ ВЫПОЛНЕНИЯ (ДОБАВЛЕНА СИСТЕМА ОЦЕНКИ)
            ...
            if not df_tasks.empty:
                ...
                if not review_tasks:
                    ...
                    for task in review_tasks:
                        ...
                        with col1:
                            # ИСПРАВЛЕНО: Теперь закрытие задачи идет через окно оценки исполнителя (1-5 звезд)
                ...
                with st.expander(f"⚖️ Спор по заданию #{task['id']}: {task['title']}"):
                    ...
                    with col1:
                        if st.button("✅ Выплатить Исполнителю", key=f"win_w_{task['id']}", use_container_width=True):
                            # Админ закрывает спор без принудительного изменения рейтинга (или можно поставить 5)

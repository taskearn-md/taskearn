...
def init_db():
    """Создает чистую базу данных с отзывами, штрафами и доказательствами выполнения"""
    ...
    # Миграция: проверяем, есть ли колонка worker_evidence (на случай, если файл БД уже существовал)
...
def send_to_review(task_id, worker_name, evidence):
    """Исполнитель отправляет работу вместе с текстом-доказательством"""
    conn = sqlite3.connect("taskearn_v13.db")
    cursor = conn.cursor()
...
st.title("🇲🇩 TaskEarn — Микрозадачи и Арбитраж")
st.write("---")
            ...
            if not review_tasks:
                ...
                for task in review_tasks:
                    with st.container():
                        ...
                        st.info(f"📞 Телефон исполнителя: **{worker_info['phone']}**")
                        
                        # ВАЖНО: Выводим доказательства выполнения от воркера
                        st.warning(f"📌 **Отчёт исполнителя (Доказательства):**\n\n>{task['worker_evidence'] if task['worker_evidence'] else 'Исполнитель не оставил текстового описания.'}")
                        ...
                        with col2:
                            ...
                            if st.session_state[show_form_key]:
                                with st.form(key=f"cancel_form_{task['id']}"):
                                    st.write("⚠️ Внимание! Ваш спор будет виден в истории отмен.")
                                    evidence = st.text_area("Ваши аргументы против отчета исполнителя:", placeholder="На скриншоте/в тексте исполнителя ложные данные, работа не доделана...")
                                    submit_cancel = st.form_submit_button("🚨 Подтвердить отмену и оспорить")
                                    
                                    if submit_cancel:
                                        if len(evidence.strip()) < 5:
                                            st.error("Пожалуйста, аргументируйте причину отмены!")
                                        else:
                    ...
                    with st.container():
                        ...
                        with col1:
                            ...
                            if task['status'] == "В работе":
                                ...
                                st.caption(f"📝 Отправленный отчет: _{task['worker_evidence']}_")
                        ...
                        with col2:
                            if task['status'] == "Доступно":
                                ...
                                # Форма для отправки доказательств
                                with st.popover("📤 Отправить отчет", use_container_width=True):
                                    with st.form(key=f"evidence_form_{task['id']}"):
                                        evidence_text = st.text_area(
                                            "Предоставьте доказательства выполнения:", 
                                            placeholder="Например: Ссылка на гугл-диск с фото результатов, текст выполненной статьи, или чек №...",
                                            help="Заказчик увидит этот текст перед проверкой. Это защитит вас в случае спора."
                                        )
                                        submit_evidence = st.form_submit_button("🚀 Отправить заказчику")
                                        
                                        if submit_evidence:
                                            if len(evidence_text.strip()) < 10:
                                                st.error("Пожалуйста, распишите доказательства подробнее (минимум 10 символов)!")
                                            else:
                                                send_to_review(task['id'], profile_data['name'], evidence_text.strip())
                                                st.success("Задание с доказательствами отправлено на проверку!")
                                                st.rerun()
                                ...
                                st.button("⏳ На проверке у заказчика", key=f"checking_{task['id']}", disabled=True, use_container_width=True)

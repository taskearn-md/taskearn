...
# 🔄 ВОССТАНОВЛЕНИЕ СЕССИИ ПРИ ПЕРЕЗАГРУЗКЕ СТРАНИЦЫ (ЧЕРЕЗ URL)
        ...
        if input_sms:
            if input_sms.strip() == st.session_state["sms_code"]:
                if st.session_state["pending_user_data"] is None:
                    ...
                    st.query_params["user_id"] = str(user_id)  # Запоминаем в URL
                    st.session_state["sms_code"] = None
                    st.session_state["pending_phone"] = None
                    ...
                    if register_user(new_id, p_data['role'], p_data['name'], p_data['phone'], p_data['about'], p_data['balance']):
                        ...
                        st.query_params["user_id"] = str(new_id)  # Запоминаем в URL
                        st.session_state["sms_code"] = None
                        st.session_state["pending_phone"] = None
                        st.session_state["pending_user_data"] = None
...
if user_data['role'] != 'admin':
    ...
    with col_s2:
        if st.button("🚪 Выйти", key="sb_logout_btn", use_container_width=True):
            st.session_state["logged_in_user_id"] = None
            st.query_params.clear()  # Очищаем URL при логауте
            st.rerun()
            
    st.sidebar.write("---")
    
    # 📊 АНАЛИТИКА В БОКОВОЙ ПАНЕЛИ
    ...
    # ⚙️ НАСТРОЙКИ ПРОФИЛЯ В БОКОВОЙ ПАНЕЛИ

...
def init_db():
    """Создает базу данных v5 с системой проверки задач, контактами и отменой"""
...
def get_profile_by_name(name):
    """Ищет телефон и данные пользователя по его имени для отображения контактов"""
    conn = sqlite3.connect("taskearn_v5.db")
    cursor = conn.cursor()
...
def cancel_task(task_id, reward):
    """Отмена задания: статус меняется, деньги возвращаются заказчику"""
    conn = sqlite3.connect("taskearn_v5.db")
    cursor = conn.cursor()
...
# Вернули заголовок с Заработком
...
if role == "💼 Заказчик":
    ...
    with tab_tasks:
        ...
        with st.form("new_task_form", clear_on_submit=True):
            ...
            task_city = st.selectbox("Nearest city/district center:", CITIES[1:])
            task_village = st.text_input("Specify the locality (village, commune, street):")
            task_category = st.selectbox("Category:", CATEGORIES[1:])
            task_reward = st.number_input("Payment to worker (MDL)", min_value=20, value=100, step=10)
            submit = st.form_submit_button("Publish and lock payment")
            ...
            if not review_tasks:
                ...
                for task in review_tasks:
                    with st.container():
                        ...
                        # Находим контакты исполнителя по его имени
                        ...
                        with col1:
                            ...
                            if st.button("✅ Принять работу", key=f"app_{task['id']}", use_container_width=True):
                                ...
                                st.rerun()
                        with col2:
                            st.write("  \n") # Отступ для выравнивания с ползунком
                            if st.button("❌ Работа не выполнена", key=f"can_{task['id']}", use_container_width=True):
                                cancel_task(task["id"], task["reward"])
                                st.warning("Задание отменено, средства возвращены на ваш баланс!")
                                st.rerun()
                        st.write("---")
...
# --- ЛОГИКА ИСПОЛНИТЕЛЯ ---
    ...
    with tab_tasks:
        ...
        if df_tasks.empty:
            ...
            # Показываем доступные И те, которые этот исполнитель уже взял на проверку
                ...
                for task in available_tasks:
                    with st.container():
                        ...
                        with col1:
                            ...
                            # Если задание уже взято в работу, показываем телефон заказчика

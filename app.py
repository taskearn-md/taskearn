...
def init_db():
    conn = sqlite3.connect("taskearn_v15.db")
    cursor = conn.cursor()
    ...
    # Таблица для балансов (добавлен кошелек admin)
    cursor.execute("""
    ...
    cursor.execute("INSERT OR IGNORE INTO balances (role, balance) VALUES ('admin', 0.0)") # Кошелек владельца
...
def approve_task(task_id, total_reward):
    """При одобрении рассчитывается комиссия: часть идет воркеру, часть — админу"""
                    ...
                    with st.container():
                        ...
                        with col_t1:
                            ...
                            if task['status'] == 'Доступно':
                                st.caption(f"🟢 Статус: `В ленте` | Исполнитель получит: {task['reward'] * (1 - COMMISSION_RATE)} MDL (с учетом комиссии сервиса)")
                ...
                for task in review_tasks:
                    ...
                    with col1:
                        if st.button("✅ Принять и оплатить", key=f"app_{task['id']}", use_container_width=True):
                            ...
                            st.rerun()
                    with col2:
                        if st.button("❌ Отклонить работу", key=f"btn_fail_{task['id']}", use_container_width=True):
                            client_dispute_task(task["id"], "Работа выполнена некорректно.")
                            st.rerun()
    ...
    with tab_tasks:
        ...
        if not df_tasks.empty:
            ...
            for task in my_current_tasks.to_dict(orient="records"):
                # Считаем, сколько реально чистыми получает исполнитель
                ...
                with st.container():
                    ...
                    with col1:
                        ...
                        st.write(f"💰 Оплата вам на руки: **{clean_reward} MDL** *(Чистыми)*")
                        st.caption(f"ℹ️ (Полная цена заказа: {task['reward']} MDL | Комиссия системы: {int(COMMISSION_RATE*100)}%)")
                    ...
                    with col2:
                        if task['status'] == "Доступно":
                            ...
                            if st.button("👍 Согласиться с отменой", key=f"agree_{task['id']}", use_container_width=True):
                                ...
                                st.rerun()
                            if st.button("⚖️ Оспорить в Арбитраж", key=f"arb_{task['id']}", use_container_width=True):
                                worker_send_to_arbitration(task['id'], "Я выполнил всё честно.")
                                st.rerun()
                    st.write("---")
...
# --- ПАНЕЛЬ АДМИНИСТРАТОРА (ВЫ) ---
    ...
    # Главная финансовая метрика владельца
    st.metric(label="💰 Чистый доход вашей платформы (Заработано на комиссии)", value=f"{balance_admin} MDL")
    st.caption("Эти деньги находятся на транзитном счету вашего приложения и готовы к выводу на ваш расчетный счет/карту.")
        ...
        if not arbitration_tasks:
            ...
            for task in arbitration_tasks:
                with st.expander(f"Спор по задаче: {task['title']}"):
                    st.write(f"Общая сумма заказа: **{task['reward']} MDL**")
                    st.write(f"Ваша комиссия в случае успеха: **{task['reward'] * COMMISSION_RATE} MDL**")
                    st.write(f"Выплата воркеру в случае успеха: **{task['reward'] * (1 - COMMISSION_RATE)} MDL**")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("Победа Исполнителя ✅ (Выплатить с комиссией)", key=f"win_w_{task['id']}", use_container_width=True):
                            approve_task(task['id'], task['reward'])
                            st.success("Решение вынесено! Исполнитель получил деньги, вам зачислился процент.")
                            st.rerun()
                    with col2:
                        if st.button("Победа Заказчика ❌ (Полный возврат)", key=f"win_c_{task['id']}", use_container_width=True):
                            conn = sqlite3.connect("taskearn_v15.db")
                            cursor = conn.cursor()
                            ...
                            conn.commit()
                            conn.close()
                            st.error("Деньги возвращены заказчику в полном объеме. Комиссия не удержана.")
                            st.rerun()
